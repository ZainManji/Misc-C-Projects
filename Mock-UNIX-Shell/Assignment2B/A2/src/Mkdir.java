...
public class Mkdir extends Command {
     ...
     * @return void
     */
    private void create(String path, JShell shell) {
        String cur;
        String dirName;
        Dir dir;
        ...
        if (path.contains("/")){//handle absolute path
            ...
            curr = Dir.searchDir(cur, shell.getRoot());
    ...
    public boolean checkParameters(String[] param, JShell shell) {
        ...
        for(int i = 0; i < param.length; i ++){
            ...
            if (!dirName.matches("[a-zA-Z1-9]*")){
                ...
                this.error = "The directory " + path.substring(path.lastIndexOf("/")) + " doesn't exist";
                return false;
            }
            else if((Dir.searchDir(path, curr)) != null){
                if (Dir.searchDir(path, curr).getDocChild(dirName) != null){
