...
public class Cp extends Command {
     ...
     * @return null as there is no text output
     */
    public String execute(String[] param, JShell s) {
        ...
        Dir parentDir = currentDir.searchDir(newPath, rootDir);

        if (oldPathNoName.length() < 1) {
            oldPathNoName = "/";
        }

        Dir oldParDir = currentDir.searchDir(oldPathNoName, rootDir);
        
        if (oldParDir.getDocChild(oldName) != null) { // triggered if oldPath refers to a document
            Document oldDoc = currentDir.searchDoc(oldPath, rootDir);
            Document newDoc = new Document(oldDoc.getName(), newPath);
            
            newDoc.writeContents(oldDoc.getContents());
            parentDir.addDoc(newDoc);
        } else {
            Dir oldDir = currentDir.searchDir(oldPath, rootDir);
     ...
     * @return void
     */
    private void duplicateTree(Dir oldDir, Dir newDir) {
     ...
     */
    private boolean checkDestinationIsDir() {
        ...
        if (!newPath.equals("/")) {
            ...
            if (currentDir.checkPathExists(newPathNoName, rootDir)) {
                Dir newParDir = currentDir.searchDir(newPathNoName, rootDir);
    ...
    private boolean checkFileExists() {
        ...
        if (currentDir.checkPathExists(oldPathNoName, rootDir)) {
            oldParDir = currentDir.searchDir(oldPathNoName, rootDir);
     ...
     * following conditions: -the path of the source directory is not the root
     * directory. -the destination path is not a file -the destination is not
     * the source -the source does not contain the destination -the destination
